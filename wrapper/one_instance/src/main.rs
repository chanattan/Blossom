...
fn main_wasm(path: &str, wasm_prg: &str) -> Result<()> {
    ...
    /*
    * The initial use of the orchestrator was to input the join query to be performed
    * as the first line of the input in the memory. However, we are now considering
    * for benchmarks purposes that the Wasm modules performs every query with given datasets
    * as input.
    */
    ...
    */
    linker
        .func_wrap("host", "get_input_size", move || -> i32 {
            1100693130 // 1100693130 // ~ 1 GiB
        })
        .expect("get_input_size function defined.");

    println!("--- Reading all datasets ---");
    
    /*
    * Define for each dataset the reading function, the single instance is static and the datasets are hardcoded.
    */
    let mut content_sizes = vec![];
    for i in SCALE_FACTORS {
        ...
        println!("Content size: {}", buf.len());
        println!("");

        /*
        * Function : Read input in the memory to location known by the Wasm module as a pointer.
                     It should be defined for each dataset since it hardcodes the data for this method.
        */
        linker.func_wrap(
            "host",
            format!("read_input_{}", i).as_str(),
            move |mut caller: Caller<'_, WasiCtx>, ptr: i32| {
                ...
                match mem.write(&mut caller, offset, bytes) {
                    Ok(_) => {
                        let end = start.elapsed().as_secs_f64();
                        println!("[Module] Time to write datasets to memory : {:?}s", end);
                    }
    ...
    .func_wrap(
        "host",
        "retrieve_output",
        move |mut caller: Caller<'_, WasiCtx>, ptr: i32, capacity: i32, size: i32| {
            let mem = match caller.get_export("memory") {
                Some(Extern::Memory(mem)) => mem,
                _ => anyhow::bail!("Failed to find host memory"),
            };
    ...
    /*
     * For each dataset we perform the benchmarks.
     */
    for i in SCALE_FACTORS {
        ...
        for _ in 0..MEAN_ALGORITHMS {
            ...
            .ok_or(0) {
                Ok(benchmark_wasm_joins) => {
                    ...
                    let content_size = content_sizes.get(index - 1).unwrap();
                    match benchmark_wasm_joins.call(&mut store, &[Val::I32(index2), Val::I32(*content_size)], results) {
                        Ok(()) => {
                            let end = start.elapsed().as_secs_f64();
...
fn main() {
    main_wasm("../../datasets", "one_instance.wasm");
